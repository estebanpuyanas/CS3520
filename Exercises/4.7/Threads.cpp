...
int main() {
    ...
    */
    ...
    mutex parallel_job_sum_lock;
    thread threads[NUM_THREADS];

    for (int i = 0; i < NUM_THREADS; ++i) {
        threads[i] = thread([&parallel_job_sum, &parallel_job_sum_lock]() {
            while (!job_queue.empty()) {
                int job_input;
                {
                    lock_guard<mutex> lock(parallel_job_sum_lock);
                    if (job_queue.empty()) break;
                    job_input = job_queue.front();
                    job_queue.pop();
                }
                parallel_job_sum += do_job(job_input);
            }
        });
    }
